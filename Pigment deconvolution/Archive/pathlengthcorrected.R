...
source("~/Desktop/Projects/QUB/Pigments/pigment.function.R")
pigment.objects <- ls()
w <- 400:700
...
s <- read.csv("~/Desktop/Projects/QUB/Pigments/corrected2.csv", header = T)
...
# Pathlength in well (z[cm] = V[cm^3]/A[cm^2]) 
z = 1
...
# Read sample weights
mass <- read.csv("~/Desktop/Projects/QUB/Pigments/mass.csv", header = T)

# Sample weights are given in mass$g and extraction volume was 10 ml; dilution is 1.
ug.g  <- sweep(mg.L, 2, 10 / mass$g, "*") # --> ug pigment / g dry tissue

# Write concentrations to csv file
write.csv(ug.g, "~/Desktop/Projects/QUB/Pigments/corrected.pigments2.csv")
